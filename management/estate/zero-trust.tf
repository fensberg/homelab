...
resource "cloudflare_zero_trust_access_policy" "members" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "tunnel.members is empty, so this policy would admit nobody and no device could enroll. List the members' emails, comma-separated, in the vault."
    }
  }
}
