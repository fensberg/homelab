...
// `estate` vault, and refuses to run if its token can see a site's own vault. The
...
1Password token that can see a site's own vault, and wipes them on the way out.
`
