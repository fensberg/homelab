...
var vaultCheckFiles = []string{
	...
	"scripts/ignite/internal/phases/checkvault.go",
	"scripts/ignite/internal/config/vaultrefs.go",
}
...
func TestVaultProbeReturnsAStatusAndNothingElse(t *testing.T) {
	root := repoRoot(t)
	path := filepath.Join(root, "scripts", "ignite", "internal", "onepassword", "probe.go")
...
func TestVaultCheckStillExists(t *testing.T) {
	...
	probe, err := os.ReadFile(filepath.Join(root, "scripts", "ignite", "internal", "phases", "checkvault.go"))
	if err != nil {
