// A module of its own, deliberately separate from scripts/ignite.
//
// scripts/ignite has zero external dependencies, and that is load-bearing:
...
// Terratest pulls in the Kubernetes client set and most of its transitive
// world; putting that in ignite's module would trade all of the above for
// test-only code that never ships.
//
// The cost is that this module cannot import homelab/ignite/internal/* - Go's
// internal/ rule is per-module. That is why the config-contract tests live
// inside ignite itself (scripts/ignite/internal/config/contract_test.go) and
