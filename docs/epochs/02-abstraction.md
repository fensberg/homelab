...
#### The site's vaults: where everything in `homelab` went

The second step retired the `homelab` vault. Every reference now names its
scope's vault. Items are named for their function, and a vendor appears only
as a `provider` value:

| Was                                                          | Now                                    | Why there                                                                       |
| ------------------------------------------------------------ | -------------------------------------- | ------------------------------------------------------------------------------- |
| `homelab/organization`, `source-control`, `alerting`         | `estate-shared/...`                    | every site needs them; none harms a sibling                                     |
| `homelab/state_backup/recipient`                             | `estate-shared/state_backup/recipient` | the public half of the one break-glass key                                      |
| `homelab/state_backup/identity`                              | `estate/state_backup/identity`         | the private half, which no site may read                                        |
| `homelab/object_storage/*`, `homelab/site0/object_storage/*` | `site0-shared/object_storage/*`        | what the estate grants the site; still account-wide until the lawyer narrows it |
| `homelab/tunnel/api_token`, `provider`                       | `site0-shared/tunnel/*`                | the same, and the same caveat                                                   |
| `homelab/site0/overlay_network/*`                            | `site0-shared/overlay_network/*`       | the same, and the same caveat                                                   |
| `homelab/tunnel/secret`, `homelab/valheim/*`                 | `site0/...`                            | the site's own: generated by it, or its workload's                              |
| `homelab/site0/name`                                         | `site0/identity/name`                  | the site's own                                                                  |
| `homelab/site0/{hypervisor,node0,database}/*`                | `site0/{hypervisor,node0,database}/*`  | the site's own; a section of one item became an item                            |

The grants sit in `site0-shared` from the start, even while they are still the
broad credentials, so the third step changes what is in them rather than where
they are. Nothing moves twice.

**One template, not one per site.** The plan said a template per site, because
one `op inject` over every site fails for a token that can read only one. With
OpenBao arriving before a second site, a second template would never exist in
the 1Password era, so `config/management.tpl.json` names `site0`'s vaults
directly. Splitting it now would have been work for a site that 1Password will
never see.

**The contractor checks its token.** `vaults.CheckSite` refuses a token unless
it sees the site's vault, and it refuses one that sees anything besides that
vault, `site0-shared` and `estate-shared`. The estate's vault, a sibling's vault
and the retired `homelab` vault are all refused before a single secret is read.
The lawyer's rule and the site's rule are both in `scripts/details/vaults`, so
the two programs cannot disagree about what a scope is.

**Restore takes the identity on stdin.** It was the only code that read the
break-glass identity, and it read it from a vault the site could see. Now the
identity is in `estate`, which the site cannot see, so a restore is run by a
person holding the estate token:
`op read op://estate/state_backup/identity | contractor restore -site site0`.
With nothing piped, it refuses before touching anything, and it does not prompt,
since a prompt for a private key invites pasting it into a scrollback. The
keypair check at Render now sees only the recipient. It cannot confirm the
identity exists, and that is the property rather than a gap: a site that could
confirm it could read it.
