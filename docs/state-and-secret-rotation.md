...
**This is built and on.** `scripts/contractor/internal/phases/encryption.go`
...
the key pair into `op://<site>-shared/object_storage/`. Rotating one is
replacing that bucket's token in the estate root, which the lawyer then
grants, followed by the site's converge to rewrite the cluster's secret. The
replace destroys the old token, so there is no second step to forget. The
lawyer has no verb for replacing one token yet; until it does, rotation means
a full `converge-estate` after the token is marked for replacement by hand.

This used to be blocked on scope: the site's admin token could create a bucket
and not the credential that reads it (`GET /accounts/{id}/tokens/permission_groups`
answered `403`). The estate's token carries **Account · API Tokens · Edit**,
which is what made the grants possible at all.
...
#    contractor check-inventory will tell you which are still missing, without
#    printing any of them.
task check-inventory SITE=site0
